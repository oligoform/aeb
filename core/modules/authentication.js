define( function( require ) {
	...
	var generateUserDeviceId = function() {
		var base = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890".split( '' );
		var user_device_id = '';
		for ( var i = 0; i < 10; i++ ) {
			user_device_id += base[Math.floor( Math.random() * base.length )];
		}
		return user_device_id;
	};

	//User device id is generated once and for all:
	if ( !authenticationData.get('user_device_id') ) {
		authenticationData.set( 'user_device_id', generateUserDeviceId() );
		authenticationData.save();
	}

	Utils.log( 'User device ID :', authenticationData.get('user_device_id') );
	...
	var sendAuthData = function( user, pass, cb_ok, cb_error ) {
		...
		if ( public_key.length ) {
			...
			var web_service_params = getAuthWebServicesParams( 'connect_user', user, true, ['encrypted', 'device_id'], {
				encrypted: encrypted,
				device_id : authenticationData.get('user_device_id')
			} );
	 ...
	 * Get user device ID
	 */
	authentication.getUserDeviceId = function() {
		return authenticationData.get('user_device_id');
	};
	 ...
	 * @param {function} cb_auth_ok     Called if the user is connected ok
		...
		if ( user_authenticated ) {
			...
			var web_service_params = getAuthWebServicesParams( 'check_user_auth', authenticationData.get( 'user_login' ), true, ['hash','hasher','device_id'], {
				hash: hash,
				hasher:hasher,
				device_id: authenticationData.get('user_device_id')
			} );
